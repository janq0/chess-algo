...
uint64
index_to_uint64(int index, int bits, uint64 m) {
    int i, j;
    uint64 result = 0ULL;
    for (i = 0; i < bits; i++) {
        j = pop_1st_bit(&m);
...
int
transform(uint64 b, uint64 magic, int bits) {
...
uint64
find_magic(int sq, int m, int bishop) {
